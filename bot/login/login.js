// set bash title
process.stdout.write("\x1b]2;SABBIR CHAT BOT\x1b\x5c");
...
const login = defaultRequire(`${process.cwd()}/sabbir-fca`).login;
...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca || global.GoatBot.config.fcaOption || {}, async function (error, api) {
                        ...
                        async function callBackListen(error, event) {
                                if (error) {
                                        if (error.type == "ready") {
                                                log.info("LISTEN_MQTT", "MQTT connected and ready!");
                                                global.statusAccountBot = 'good';
                                                return;
                                        }
                                ...
                                global.responseUptimeCurrent = responseUptimeSuccess;
                                global.statusAccountBot = 'good';
                                log.info("EVENT", `Received: type=${event.type} | senderID=${event.senderID || event.userID} | threadID=${event.threadID}`);
